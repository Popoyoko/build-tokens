...
const convertFlutterGradient = (tokenValue) => {
    ...
    colorsMap(["#eaf", "#afe"]);
    return `Gradient.linear(begin: ${begin}, end: ${end}, colors: <Color> [${colorsMap(colors)}], tileMode: TileMode.clamp)`;
};
const convertColors = (platform = "web", tokenValue) => {
    if (tokenValue.startsWith("linear-gradient")) {
        if (platform === "app") {
            return `${convertFlutterGradient(tokenValue)} // Currently not fully supporting graident for app`;
        }
        else {
            return tokenValue;
        }
